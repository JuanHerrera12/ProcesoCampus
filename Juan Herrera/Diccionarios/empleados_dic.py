...
def listarEmpl(dicEmpleados):
    for i, d in dicEmpleados.items():
        print(f'ID: {i}')
        print(f'Nombre: {d["nombre"]}')
        print(f'Horas Trabajadas: {d["HorasTrabajadas"]}')
        print(f'Valor Horas Trabajadas: {d["ValorHora"]}')
...
def eliminarEmpleado(dicEmpleados):
    ...
    idEmpl =  leerIDEmpl()
    elimEmpleado = buscarEmpleado(dicEmpleados, idEmpl)
    if elimEmpleado == False: 
        print("El Empleado con ese código no ha sido ingresado.")
        input()
        return
    
    dicEmpleados.pop(idEmpl)
    print("El empleado ha sido eliminado satisfacotriamente")
    input("\n Presione cualquier tecla para volver al menu...")
